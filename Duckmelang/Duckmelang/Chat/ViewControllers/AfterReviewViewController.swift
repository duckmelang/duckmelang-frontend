...
class AfterReviewViewController: UIViewController {
    ...
    private func getReviewInformation() {
        Task {
            do {
                startLoading()
                guard let oppositeId = self.oppositeId, let postId = self.postId else { return }
                
                let result = try await networkService.getReviewsInformation(memberId: oppositeId, postId: postId)
                self.applicationId = result.applicationId
                
                // OtherPageTopView에 데이터 반영
                DispatchQueue.main.async {
                    // 후기글 작성 페이지 내 관련 정보 업데이트
                    self.afterReviewView.detailMain.updateView(info: result)
                    // 네비바 타이틀 업데이트
                    self.navigationItem.title = "\(result.name) 님의 페이지"
                }
    ...
    private func postReview() {
        Task {
            do {
                ...
                let _ = try await networkService.postReviews(reviewData: newReview)
