...
final class SocketManager {
    ...
    // 2. 메시지를 수신하는 함수
    func receiveMessage(completion: @escaping (Result<ReceivedMessage, Error>) -> Void) {
            ...
            case .success(let message):
                ...
                case .data(let data):
                    do {
                        ...
                        let decodedMessage = try JSONDecoder().decode(ReceivedMessage.self, from: data)
                ...
                case .string(let string):
                    ...
                    if let jsonData = string.data(using: .utf8) {
                        do {
                            let decodedMessage = try JSONDecoder().decode(ReceivedMessage.self, from: jsonData)
    ...
    // 3. 메시지를 송신하는 함수
    func sendMessage(messageRequest: ReceivedMessage, completion: @escaping (Result<ReceivedMessage, Error>) -> Void) {
        do {
